  ...
  return (
    <>
      {currentPosts.map((item) => {
        return <Post key={item.id} post={item} nextPost={nextPostMap.get(item.id)} />;
      })}
    </>
  );
};
...
const Post = ({ post, nextPost }) => {
  ...
  const readTimeText = readTime > 1 ? `${readTime} min read` : 'less than 1 min read';
          ...
          <li key={post.id}>
            ...
            {post.type === 'info' && (
              <div className={styles.smallContainer}>
                ...
                {post.video && post.video !== null && <VideoPreview src={post.video} />}
              </div>
            )}
          </li>
        ))}
